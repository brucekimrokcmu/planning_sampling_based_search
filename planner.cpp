...

{
	...
	*planlength = 0;
	...
	if (whichPlanner == 0) { //RRT
		std::vector<std::vector<double>> path;
		const double eps = 0.3*PI;
		const double stepIters = 50;
		const double goalTol = 1.7;
	...
	if (whichPlanner == 2){ // RRT*
		std::vector<std::vector<double>> path;
		const double eps = 0.3*PI;
		const double stepIters = 50;
		const double goalTol = 1.5;
